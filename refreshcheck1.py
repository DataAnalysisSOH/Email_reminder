...
# define the require function from car_donation_report
...
def sqlQuering():
    ...
    # We are Fetching the last update date and time from the specific cell in the google sheet
    last_update_cell="C1"
    # taking care of the result variable
    result = spreadsheet_service.spreadsheets().values().get(
        spreadsheetId=spreadsheet_Id, range=last_update_cell).execute()
    # taking care od the last update time, Assuming the time is in the first row and first columns
    last_update_time_str = result.get('values',[])[0][0] 
    last_update_time = datetime.datetime.strptime(last_update_time_str, "%Y-%m-%dT%H:%M:%SZ")

    # We are Calcuate the time difference between now and the last update time
    time_difference = datetime.datetime.now(pytz.timezone('America/Los_Angeles')) - last_update_time

    # We are comparing the time difference with one day (24 hours)
    if time_difference.total_seconds() < 24 * 60 * 60:
        print("The specific cell has been updated within one day. No need to update again.")
        return

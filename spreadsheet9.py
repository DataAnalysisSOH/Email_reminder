...
def main():
    ...
    try:
        ...
        refresh_time_cell = worksheet.acell("C1").value
        # We are Parse the refresh time into datetime object (modify the format needed)
        refresh_time = datetime.datetime.strptime(refresh_time_cell,"%Y-%m-%d %H:%M:%S")
        # We are Calculating the time until the next refresh
